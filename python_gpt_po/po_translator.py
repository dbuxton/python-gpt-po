...
class TranslationService:
    ...
    def validate_openai_connection(self):
        ...
        try:
            test_message = {
                "role": "system",
                "content": "Test message to validate connection.",
            }
            self.config.client.chat.completions.create(
                model=self.config.model, messages=[test_message]
            )
    ...
    def translate_bulk(self, texts, target_language, po_file_path, current_batch):
        ...
        for i, _ in enumerate(
            range(0, len(texts), self.batch_size), start=current_batch
        ):
            batch_texts = texts[i : i + self.batch_size]
            batch_info = f"File: {po_file_path}, Batch {i}/{self.total_batches}"
            batch_info += f" (texts {i + 1}-{min(i + self.batch_size, len(texts))})"
            translation_request = (
                f"Please translate the following texts from English into {target_language}. "
            ...
            while retries:
                try:
                    ...
                    self.perform_translation(
                        translation_request, translated_texts, batch=True
                    )
                    break
                except Exception as e:  # pylint: disable=W0718
                    ...
                    if retries <= 1:
                        logging.error(
                            "Maximum retries reached for %s. Skipping this batch.",
                            batch_info,
                        )
    ...
    def perform_translation(self, translation_request, translated_texts, batch=False):
        ...
        completion = self.config.client.chat.completions.create(
            model=self.config.model, messages=[message]
        )
    ...
    def scan_and_process_po_files(self, input_folder, languages):
        ...
        for root, _, files in os.walk(input_folder):
            for file in filter(lambda f: f.endswith(".po"), files):
                ...
                logging.info(
                    "Discovered .po file: %s", po_file_path
        ...
        try:
            ...
            if not file_lang or file_lang not in languages:
                if self.config.folder_language:
                    ...
                    inferred_lang = next(
                        (part for part in folder_parts if part in languages), None
                    )
                    logging.info(
                        "Attempting to infer language for .po file: %s", po_file_path
                    )
                    if inferred_lang:
                        file_lang = inferred_lang
                        logging.info(
                            "Inferred language for .po file: %s as %s",
                            po_file_path,
                            file_lang,
                        )
                    else:
                        logging.warning(
                            "Skipping .po file due to inferred language mismatch: %s",
                            po_file_path,
                        )
                        return
                else:
                    logging.warning(
                        "Skipping .po file due to language mismatch: %s", po_file_path
                    )
                    return
    ...
    def disable_fuzzy_translations(self, po_file_path):
        ...
        try:
            ...
            logging.error(
                "Error while disabling fuzzy translations in file %s: %s",
                po_file_path,
                e,
            )
...
def main():
    ...
    parser.add_argument(
        "--folder", required=True, help="Input folder containing .po files"
    )
    parser.add_argument(
        "--lang",
        required=True,
        help="Comma-separated language codes to filter .po files",
    )
    ...
    parser.add_argument(
        "--bulksize", type=int, default=50, help="Batch size for bulk translation"
    )
    parser.add_argument(
        "--model",
        default="gpt-4o-mini",
    ...
    parser.add_argument(
        "--folder-language",
        action="store_true",
        help="Set language from directory structure",
    )
    parser.add_argument(
    ...
    if not translation_service.validate_openai_connection():
        logging.error(
            "OpenAI connection failed. Please check your API key and network connection."
        )
        return
